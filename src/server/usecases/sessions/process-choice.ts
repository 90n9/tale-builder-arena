...
import {
  ...
  type StoryRequest,
  AdvanceStoryResult,
} from '@/server/usecases/story/advance-story';
...
export type ProcessChoiceResult =
  | {
      kind: 'success';
      session: Session & { story: { slug: string; storyJsonUrl: string | null } };
      result: Extract<AdvanceStoryResult, { kind: 'success' }>['body'];
    }
  | { kind: 'not_found' }
  | { kind: 'forbidden' }
  ...
  try {
    ...
    if (result.kind === 'success') {
      ...
      const currentHistory = Array.isArray(session.history)
        ? (session.history as SessionHistoryEntry[])
        : [];
      const newHistory: SessionHistoryEntry[] = [
        ...currentHistory,
        {
