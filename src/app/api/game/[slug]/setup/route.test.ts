...
describe("GET /api/game/[slug]/setup", () => {
  ...
  it("combines base attributes with race and class bonuses", async () => {
    ...
    expect(body.attributes.map((attr: { id: string }) => attr.id)).toEqual(["str", "int", "agi", "cha", "luk"]);
    expect(body.races.some((race: Record<string, unknown>) => "attribute_bonus" in race)).toBe(false);
    expect(body.classes.some((cls: Record<string, unknown>) => "starting_bonus" in cls)).toBe(false);
  });
});
